    ...
    StoreContext context, IConfiguration config, ILogger<PaymentsController> logger) : BaseApiController
{
    ...
    public async Task<ActionResult<BasketDto>> CreateOrUpdatePaymentIntent()
    {
        ...
        basket.PaymentIntentId ??= intent.Id; // ?? represents if null
    ...
    private async Task HandlePaymentIntentSucceeded(PaymentIntent intent)
    {
        var order = await context.Orders
            ...
            .FirstOrDefaultAsync(x => x.PaymentIntentId == intent.Id)
                ?? throw new Exception("Order not found");
        if (order.GetTotal() != intent.Amount) //ovoj kod e za da ako nekoj stai u kopra u drug tab neso da ne moze da go zeme
        {
